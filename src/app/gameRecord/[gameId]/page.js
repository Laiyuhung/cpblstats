...
export default function GameRecord({ params }) {
  ...
  return (
    ...
    <div className="max-w-6xl mx-auto p-4">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="bg-gray-50 p-4 rounded-lg mb-6">
                <h2 className="text-xl font-bold mb-4">出場異動</h2>
      ...
      {activeTab === 'changes' && (
        <div>
          <h2 className="text-xl font-bold mb-4">出場異動紀錄</h2>
          <table className="table-auto w-full border border-gray-300 text-sm mb-4">
            <thead>
              <tr>
                <th className="border px-2 py-1">局數</th>
                <th className="border px-2 py-1">半局</th>
                <th className="border px-2 py-1">事件</th>
                <th className="border px-2 py-1">內容</th>
              </tr>
            </thead>
            <tbody>
              {playByPlay.filter(p => ['pitching_change','substitute_batter'].includes(p.result)).map((p, i) => (
                <tr key={i}>
                  <td className="border px-2 py-1 text-center">{p.inning}</td>
                  <td className="border px-2 py-1 text-center">{p.half_inning === 'top' ? '上' : '下'}</td>
                  <td className="border px-2 py-1 text-center">
                    {p.result === 'pitching_change' ? '換投' : p.result === 'substitute_batter' ? '代打' : p.result}
                  </td>
                  <td className="border px-2 py-1 text-center">
                    {p.result === 'pitching_change' && p.pitcher_name && `新投手：${p.pitcher_name}`}
                    {p.result === 'substitute_batter' && p.batter_name && `第${p.at_bat}棒 代打：${p.batter_name}`}
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
          <h3 className="font-bold mb-2">目前打序</h3>
          <div className="grid grid-cols-2 gap-8">
            <div>
              <h4 className="font-bold mb-1">客隊</h4>
              <ul className="list-disc pl-4">
                {awayBatters.map(b => (
                  <li key={b.order}>第{b.order}棒 {b.position?.toUpperCase()} {b.name}</li>
                ))}
              </ul>
            </div>
            <div>
              <h4 className="font-bold mb-1">主隊</h4>
              <ul className="list-disc pl-4">
                {homeBatters.map(b => (
                  <li key={b.order}>第{b.order}棒 {b.position?.toUpperCase()} {b.name}</li>
                ))}
